...
- `polylog` broadcasts over `z` — the middle branch keeps its 60 powers of `log z` on a _trailing_ axis, so they cannot collide with the caller's own. `jax.vmap` still works and agrees bit for bit.
