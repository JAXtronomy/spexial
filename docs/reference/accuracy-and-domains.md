...
`comb` : Returns `0` for `k > N`, `k < 0` and `N < 0`, matching `scipy.special.comb`. `comb(inf, 0)` is `1` and `comb(inf, k >= 1)` is `inf`, as in SciPy. The inexact variant only; there is no `exact=True` path. The $N \le 170$ domain is a real ceiling, not a formality: the value comes from $\Gamma\ln(N+1) - \Gamma\ln(N-k+1)$, which cancels catastrophically for large $N$ — `comb(1e10, 2)` is already off by $3.7\times10^{-6}$, and `comb(1e300, 2)` returns `1.0` where SciPy returns `inf`.
