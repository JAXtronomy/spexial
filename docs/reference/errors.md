...
| --- | --- | --- |
| `Li(0, z)`, `Li(-1, z)` | `ValueError` | `Li is only implemented for integer order n >= 1, got 0` |
| `Li(n, z)` with complex `z` | `ValueError` | `Li is only implemented for real z, got dtype complex128` |
| `Li(2.0, z)` — non-integer order | `TypeError` | see [below](#a-non-integer-order) |
| `Li(n, z)` with array `z` | `TypeError` | `pow got incompatible shapes for broadcasting: (2,), (59,)` |
...
The last four are raised by JAX, not by `spexial`, and their wording is therefore not ours to promise. They are listed because they are what you will actually see, and because each corresponds to a documented restriction: `Li` takes a scalar `z`, `eval_gegenbauers` takes a scalar `x`, and the Bessel and zeta functions are real-only.

### A non-integer order

`Li(2.0, z)` — a whole-number `float` where an `int` is meant — is the most likely of these mistakes, and what you see depends on whether runtime type checking is switched on.
...
spexial._src.polylog.Li.  The problem arose whilst typechecking parameter 'n'.
Actual value: 2.0
...
That message names neither `Li` nor the argument at fault. If you are debugging an opaque error from inside a trace, turning the checker on is usually the fastest way to find out which argument caused it:
...
| `Li(n, z)` with `n > 60` and $\lvert z \rvert \ge 2$ | `nan` — the inversion branch needs $B_{60}$ |
